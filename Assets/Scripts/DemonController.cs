...
public class DemonController : MonoBehaviour
{
    ...
    public DemonBehaviorMode behaviorMode = DemonBehaviorMode.Idle;
    
    [Header("Patrol Settings - Solo para Patrol Mode")]
    [Tooltip("Centro del área de patrulla (usualmente el DemonSpawner)")]
    public Transform patrolCenter;
    [Tooltip("Radio del área donde patrullará el demon")]
    ...
    public float detectionRadius = 15f; // Radio de detección más grande que zombie
    public float fieldOfView = 150f; // Campo de visión más amplio en grados
    ...
    public AudioClip demonIdleSound; // Sonido que se repite constantemente
    ...
    public AudioClip demonDeathSound; // Sonido de muerte
    [Range(0f, 1f)] public float deathSoundVolume = 1f;

    [Header("Health Bar Settings")]
    public GameObject healthBarPrefab; // Prefab de la barra de vida
    public Vector3 healthBarOffset = new Vector3(0, 2.5f, 0); // Altura sobre el demon

    [Header("References")]
    public Transform player;
    public Transform eyePosition; // Posición desde donde "ve" el demon (cabeza)
    public Animator demonAnimator;
    public NavMeshAgent agent;
    public DemonRagdollController demonRagdoll;
    ...
    void Start()
    {
        ...
        if (demonRagdoll == null)
            demonRagdoll = GetComponentInChildren<DemonRagdollController>();

        // Configurar AudioSource para los sonidos del demon
        ...
        else if (agent != null)
        {
            ...
            agent.speed = (behaviorMode == DemonBehaviorMode.Patrol) ? patrolSpeed : runSpeed;
            ...
            agent.stoppingDistance = (behaviorMode == DemonBehaviorMode.Patrol) ? 0.5f : stoppingDistance;
        ...
        // Si no hay eyePosition asignado, usar la posición del demon + offset
        if (eyePosition == null)
        {
        ...
        if (healthBarPrefab != null)
        {
            ...
            DemonHealthBar healthBarScript = healthBarInstance.GetComponent<DemonHealthBar>();
            
            if (healthBarScript != null)
            {
                healthBarScript.demon = this;
        ...
        if (behaviorMode == DemonBehaviorMode.Patrol)
        {
            // Si no hay patrol center asignado, usar la posición inicial del demon
    ...
    void Update()
    {
        ...
        // Reproducir sonidos idle del demon
        PlayIdleSound();
        ...
        if (isPlayingDetectionAnimation)
        {
            ...
            // Mantener al demon quieto durante la animación
            ...
            if (detectionAnimationTimer >= detectionAnimationDuration)
            {
                ...
                if (demonAnimator != null)
                {
                    demonAnimator.SetBool("isWalking", false); // FORZAR a false para Run
                    demonAnimator.SetLayerWeight(1, 1f); // Asegurar que Walk layer esté activo
                    demonAnimator.SetLayerWeight(2, 0f); // Desactivar detectPlayer layer
                }
        ...
        else
        {
            ...
            switch (behaviorMode)
            {
                ...
                case DemonBehaviorMode.Patrol:
                    // Modo Patrol: Caminar por el área
                    PatrolArea();
                    break;
                    
                case DemonBehaviorMode.Horde:
    ...
    void PlayIdleSound()
    {
        ...
        if (isDead || demonIdleSound == null || audioSource == null || Time.timeScale == 0f)
            return;

        // Verificar si es tiempo de reproducir el siguiente sonido
        if (Time.time >= nextIdleSoundTime)
        {
            audioSource.PlayOneShot(demonIdleSound, idleSoundVolume);
    ...
    void DetectPlayer()
    {
        ...
        if (hasPlayedDetectionAnimation)
        {
            ...
            if (behaviorMode == DemonBehaviorMode.Patrol && patrolCenter != null)
            {
        ...
        if (distanceToPlayer <= detectionRadius)
        {
            ...
            {
                // Verificar que no haya obstáculos entre el demon y el jugador (Raycast)
                RaycastHit hit;
                if (Physics.Raycast(eyePosition.position, directionToPlayer, out hit, detectionRadius, ~(1 << LayerMask.NameToLayer("Hitbox"))))
                {
                    ...
                    if (hit.transform == player || hit.transform.root == player.root)
                    {
                        ...
                        if (!playerDetected && !hasPlayedDetectionAnimation)
                        {
                            ...
                            if (demonAnimator != null)
                            {
                                demonAnimator.SetLayerWeight(2, 1f); // detectPlayer layer
                                demonAnimator.SetLayerWeight(1, 0f); // Walk layer apagado
                            }
                        ...
                        else if (!playerDetected && hasPlayedDetectionAnimation)
                        {
                            ...
                            {
                                demonAnimator.SetBool("isWalking", false); // FORZAR a false para Run
                                demonAnimator.SetLayerWeight(1, 1f);
                                demonAnimator.SetLayerWeight(2, 0f);
                            }
                    ...
                    {
                        // Hay un obstáculo entre el demon y el jugador (solo perder detección si no había detectado antes)
                        if (!hasPlayedDetectionAnimation)
                            playerDetected = false;
                    }
                }
    ...
    void UpdateAnimator(bool running, bool walking)
    {
        ...
        if (running)
        {
            ...
            demonAnimator.SetLayerWeight(1, 1f); // Walk layer - animación de correr
            demonAnimator.SetLayerWeight(2, 0f); // detectPlayer layer - apagar detección
        ...
        else if (walking)
        {
            ...
            demonAnimator.SetLayerWeight(1, 1f); // Walk layer - animación de caminar
            demonAnimator.SetLayerWeight(2, 0f); // detectPlayer layer - apagar detección
        ...
        else
        {
            ...
            demonAnimator.SetLayerWeight(1, 0f); // Walk layer
            demonAnimator.SetLayerWeight(2, 0f); // detectPlayer layer
            demonAnimator.SetBool("isWalking", false);
        }
    }
    ...
    void Die()
    {
        ...
        if (demonAnimator != null && !deathAnimationPlayed)
        {
        ...
        if (audioSource != null && demonDeathSound != null)
        {
            audioSource.Stop(); // Detener sonidos actuales
            audioSource.PlayOneShot(demonDeathSound, deathSoundVolume);
        }
    ...
    void OnDestroy()
    {
        // Destruir la barra de vida cuando el demon se destruya
    ...
    void GenerateNewPatrolPoint()
    {
        ...
        {
            // Si no encuentra un punto válido, intentar desde la posición actual del demon
